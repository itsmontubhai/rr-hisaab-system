...
export function calculateDistribution(tradingIdEntity, amount, allEntities, percentHistory, partnerAllocations, partners) {
  ...
  // Partners: chain ke kisi bhi level pe ho sakte hain — Master, Broker Group, ya Trading ID khud
  const chainIds = [tradingIdEntity.id, ...chain.map((c) => c.id)]
  const relevantAllocations = partnerAllocations.filter(
    (a) => !a.effective_to && chainIds.includes(a.entity_id)
  )
...
export function getDescendantTradingIds(entityId, allEntities) {
  const direct = allEntities.filter((e) => e.parent_id === entityId)
  let result = direct.filter((e) => e.type === 'trading_id')
  direct.forEach((d) => {
    if (d.type !== 'trading_id') {
      result = result.concat(getDescendantTradingIds(d.id, allEntities))
    }
  })
  return result
}
